...
class QnckSpider(EpaperSpider):
    name = '青年参考'
    base_url = 'http://qnck.cyol.com/html/{}/nbs.D110000qnck_01.htm'
    date_format = '%Y-%m/%d'

    def parse(self, response):
        # 提取各个版面链接
        hrefs = response.xpath('//div[@id="pageList"]/ul/li/a/@href').extract()
    ...
    def parse_page(self, response):
        ...
        image = response.urljoin(response.xpath('//div[@class="ban"]//img/@src').extract_first(''))
    ...
    def parse_article(self, response):
        ...
        image_links = response.xpath('//div[@class="c_c BSHARE_POP"]//img/@src').extract()
        article['origin_image_list'] = [response.urljoin(url) for url in image_links]
        images_desc_el = response.xpath('//div[@class="tushuo"]/text()') if '//div[@class="tushuo"]/text()' else ''
