...
class BjbSpider(EpaperSpider):
    name = '北京商报'
    base_url = 'http://epaper.bjbusiness.com.cn/site1/bjsb/html/{}/node_205.htm'
    date_format = '%Y-%m/%d'
    ...
    def parse_page(self, response):
        # 提取版面信息
        banner = response.xpath('//td[@class="px12" and @align="left"]//text()').extract_first('').strip()
        number, name = self.split_banner(banner, '')
        image = response.urljoin(response.xpath('//img[@usemap="#PagePicMap"]/@src').extract_first(''))
        pdf_url = response.urljoin(response.xpath('//a[contains(@href, "pdf.pdf")]/@href').extract_first('')) if '//a[contains(@href, "pdf.pdf")]/@href' else ''
        article_hrefs = [response.urljoin(url) for url in response.xpath('//a[contains(@href, "content")]/@href').extract()]
    ...
    def parse_article(self, response):
        # 提取文章信息
        article = ArticleItem()
        article['title'] = response.xpath('//tr[@valign="top"]/td/strong/text()').extract_first('').strip()
