...
class bjwbBjdSpider(EpaperSpider):
    name = '北京晚报'
    base_url = 'http://bjwb.bjd.com.cn/html/{}/node_113.htm'
    date_format = '%Y-%m/%d'

    def parse(self, response):
        # 提取各个版面链接
        hrefs = response.xpath('//div[@class="hidenPage"]/li//a/@href').extract()
    ...
    def parse_page(self, response):
        # 提取版面信息
        banner = response.xpath('//div[@class="hidenPage"]/li//a//text()').extract_first('').strip()
        number, name = self.split_banner(banner, '')
        image = response.urljoin(response.xpath('//img[@id="Bantu"]/@src').extract_first(''))
        pdf_url = response.urljoin(response.xpath('//*[@id="downpdfLink"]/a/@href').extract_first('')) if '//*[@id="downpdfLink"]/a/@href' else ''
        article_hrefs = [response.urljoin(url) for url in response.xpath('//div[@class="main-list"]//a/@href').extract()]
    ...
    def parse_article(self, response):
        ...
        article['subtitle'] = response.xpath('//div[@class="article"]/h2[2]//text()').extract_first('').strip() if '//div[@class="article"]/h2[2]//text()' else ''
        ...
        images_desc_el = response.xpath('//*[@id="list"]/div/div[2]/div/table//p/text()') if '//*[@id="list"]/div/div[2]/div/table//p/text()' else ''
        ...
        author = response.xpath('//div[@class="info"]/span[last()]/text()').extract_first('').strip() if '//div[@class="info"]/span[last()]/text()' else ''
        article['author'] = self.handles_author(author)
        article['reporter'] = response.xpath('').extract_first('').strip() if '' else ''
        article['keywords'] = response.xpath('').extract_first('').strip() if '' else ''
        source = response.xpath('//div[@class="info"]/span[1]/text()').extract_first('').strip() if '//div[@class="info"]/span[1]/text()' else ''
        article['source'] = self.handles_source(source)
