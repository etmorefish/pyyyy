...
class XXNETSpider(EpaperSpider):
    name = '湘西团结报'
    base_url = 'http://www.xxnet.com.cn/szb/tjbpc/{}/l01.html'
    date_format = '%Y%m/%d'

    def parse(self, response):
        # 提取各个版面链接
        hrefs = response.xpath('//div[@class="nav-list"]/ul/li/a/@href').extract()
    ...
    def parse_page(self, response):
        # 提取版面信息
        banner = response.xpath('//div[@class="tabs"]/h3/text()').extract_first('').strip()
        number, name = self.split_banner(banner, '')
        image = response.urljoin(response.xpath('//div[@class="newspaper-pic"]/img[@class="preview"]/@src').extract_first(''))
        pdf_url = response.urljoin(response.xpath('').extract_first('')) if '' else ''
        article_hrefs = [response.urljoin(url) for url in response.xpath('//div[@class="news-list"]/ul/li/a/@href').extract()]
    ...
    def parse_article(self, response):
        # 提取文章信息
        article = ArticleItem()
        article['title'] = response.xpath('//*[@id="Title"]/text()').extract_first('').strip()
        ...
        image_links = response.xpath('//div[@id="ozoom"]//img/@src').extract()
