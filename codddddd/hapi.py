...
def SLIT_INHERENT( x,g,p1=0.3,p2=0.5,p3=0.1,p4=-0.8,p5=0.8 ):
    '''
    Inherent Instrumental (slit) function.
    where γ is a slit width or the instrumental resolution.
    '''
    index_left = x < -p1
    index_greater_p1 = x >= -p1
    index_less_p4 = x <= -p4
    index_between_p1p4 = index_greater_p1 * index_less_p4
    index_greater_p4 = x > -p4
    index_less_lp4 = x <= (1 - p4/2)
    index_middle = index_greater_p4 * index_less_lp4
    index_left = x > (1-p4/2)
    y = zeros(len(x))
    y[index_left] = 0
    y[index_between_p1p4] = p2 + p3 * (x[index_between_p1p4] + p1)**2
    y[index_middle] = p5
    y[index_left] = 0
